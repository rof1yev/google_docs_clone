...
import { ToolbarSectionItem } from "@/interface";
import {
  AlignCenterIcon,
  AlignJustifyIcon,
  AlignLeftIcon,
...
import { type ColorResult, SketchPicker, Color } from "react-color";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className="flex items-center gap-x-0.5">
      <button
        onClick={decrement}
        className={cn(
          "size-7 shrink-0 flex items-center justify-center rounded-sm hover:bg-neutral-200/80"
        )}
      >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleImageUrlSubmit}>
              Insert
            </Button>
          </DialogFooter>
        </DialogContent>
...
const FontFamilyButton = () => {
  ...
  const changeFontFn = useCallback(
    (value: string) => editor?.chain().focus().setFontFamily(value).run(),
    []
  );
...
export const Toolbar = () => {
  ...
  const sections: ToolbarSectionItem[][] = useMemo(
    () => [
      [
        {
          label: "Undo",
  ...
  return (
    <div className="bg-[#f1f4f9] px-2.5 py-0.5 rounded-[24px] min-h-[40px] flex items-center gap-x-0.5 overflow-x-auto">
      {sections[0].map((item: ToolbarSectionItem) => (
      ...
      <FontSizeButton />
      <Separator orientation="vertical" className="h-6 bg-neutral-300" />
      {sections[1].map((item: ToolbarSectionItem) => (
        <ToolbarButton key={item.label} {...item} />
      ))}
      <TextColorButton />
      ...
      <LineHeightButton />
      <ListButton />
      {sections[2].map((item: ToolbarSectionItem) => (
        <ToolbarButton key={item.label} {...item} />
      ))}
    </div>
  );
