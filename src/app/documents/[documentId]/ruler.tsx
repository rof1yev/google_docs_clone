...
export const Ruler = () => {
  ...
  const handleLeftMouseDown = () => setIsDroppingLeft(true);
  const handleRightMouseDown = () => setIsDroppingRight(true);

  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    const PAGE_WIDTH: number = 816;
    const MINIMUM_SPACE: number = 100;

    if ((isDroppingLeft || isDroppingRight) && rulerRef.current) {
  ...
  const handleMouseUp = () => {
    setIsDroppingLeft(false);
    setIsDroppingRight(false);
  };
